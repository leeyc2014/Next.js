...
export default async function RestaurantDetail({ params }: RestaurantDetailProps) {
    ...
    const homepage = restaurant?.HOMEPAGE_URL;
    ...
    return (
        <div className="w-full h-full flex flex-col">
            ...
            <p className="pl-5">{restaurant?.GUGUN_NM}</p>
            <div className="relative w-7/10 h-80 bg-gray-200 rounded-sm m-5">
                {restaurant?.MAIN_IMG_NORMAL ? (<Image src={restaurant.MAIN_IMG_THUMB} alt={restaurant.TITLE} fill sizes="(max-width: 768px) 100vw, 50vw" style={{ objectFit: 'cover' }} priority />) : (<div className="w-full flex justify-center items-center">이미지 없음</div>)}
            </div>
            ...
            <h2 className="p-5 text-gray-500">상세 설명</h2>
            <div className="pl-5">
                {description}
            </div>
